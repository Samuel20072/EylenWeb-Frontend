...
export class ProblemsSection implements OnInit {
  problems = [
    {
      key: 'problem1',
      number: 'PROBLEMA 1',
  ...
  constructor(private sitieMediaService: SitieMediaService) { }

  ngOnInit(): void {
    this.sitieMediaService.findAll().subscribe(mediaList => {
      this.problems.forEach(problem => {
        // Find media that matches the problem key in either section or title
        const media = mediaList.find(m => m.section === problem.key || m.title === problem.key);
        if (media) {
          problem.imageSrc = media.url;
        }
      });
    });
  }
