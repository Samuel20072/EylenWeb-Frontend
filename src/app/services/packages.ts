...
// El archivo (comprobante) se envía como 'multipart/form-data' por separado.
...
export class AdminPackageService {
  ...
  private apiUrl2 = 'http://localhost:3000/packages';
...
export class UserPackageService {
  ...
  purchasePackage(id: number, file: File, notes?: string): Observable<PackagePurchase> {
    ...
    // NestJS (Multer) los deserializará en el @Body() dto
    if (notes) {
      formData.append('notes', notes);
    }

    // Mapea a la ruta: POST /packages/:id/purchase
    // HttpClient de Angular maneja el Content-Type: multipart/form-data automáticamente al usar FormData.
